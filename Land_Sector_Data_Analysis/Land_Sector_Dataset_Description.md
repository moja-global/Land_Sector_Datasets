...
The directories with Level 2 contain boundaries at the country level including the Global Ecological Zone boundaries for each country as well as the World Soil Resourcess' boundaries;
...
The directories with Level 4 contain boundaries of each state-level grouped by country as well as the Ecological Zone boundaries and the World Soil Resources boundary for each state. At Level 4:
...
This directory contains one file for the global ecological zone as well as 02 subdirectories that contain the JSON files of ECological zones grouped by country and state respectively. The attributes of the files in this section are the same as those of the GEZ files of section 1.1.1.
...
This directory contains 04 principal subdirectories(**datamask, gain, lossy year, and tree cover**). In these directories are GeoTiff tiles with the names implying the interest
...
Contains files whose notation indicates the country of interest. The properties are the same as in 1.1.1.
